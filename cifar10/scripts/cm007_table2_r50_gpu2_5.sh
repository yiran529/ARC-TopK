...
LOG_DIR="${REPO_DIR}/output/CM007-table2-r50-gpu2-5"
TARGET_GPUS="2,3,4,5"
SEED=1410
...
echo "[CM007] Waiting for CM006 ResNet-18 experiment to finish..."
while tmux has-session -t cm006-table2-r18-gpu2-5 2>/dev/null; do
    sleep 30
done
echo "[CM007] CM006 has finished."

echo "[CM007] Waiting for CIFAR-10 extraction..."
...
echo "[CM007] CIFAR-10 is ready."

wait_for_target_gpus() {
    echo "[CM007] Waiting for GPU ${TARGET_GPUS} with utilization < 30% and free memory >= 4 GiB..."
    while true; do
        ...
        if [ "${READY_COUNT}" -eq 4 ]; then
            echo "[CM007] Selected CUDA_VISIBLE_DEVICES=${TARGET_GPUS}"
            return
        fi
        sleep 30
    done
...
wait_for_target_gpus
CUDA_VISIBLE_DEVICES="${TARGET_GPUS}" WANDB_MODE=online WANDB_PROJECT=cifar10_resnet50_table2 PYTHONPATH=. \
    "${TORCHRUN_BIN}" --standalone --nproc_per_node=4 --master-port=29718 \
    cifar10/run_cifar10_resnet50.py \
...
wait_for_target_gpus
CUDA_VISIBLE_DEVICES="${TARGET_GPUS}" WANDB_MODE=online WANDB_PROJECT=cifar10_resnet50_table2 PYTHONPATH=. \
    "${TORCHRUN_BIN}" --standalone --nproc_per_node=4 --master-port=29719 \
    cifar10/run_cifar10_resnet50.py \
...
wait_for_target_gpus
CUDA_VISIBLE_DEVICES="${TARGET_GPUS}" WANDB_MODE=online WANDB_PROJECT=cifar10_resnet50_table2 PYTHONPATH=. \
    "${TORCHRUN_BIN}" --standalone --nproc_per_node=4 --master-port=29720 \
    cifar10/run_cifar10_resnet50.py \
    --optimizer muon --lr 0.02 --weight_decay 5e-4 \
    --muon_scalar_lr 1e-3 --muon_scalar_weight_decay 5e-4 \
    --muon_mu 0.95 --muon_epsilon 1e-8 --muon_scalar_eps 1e-8 \
...
echo "[CM007] All three ResNet-50 runs finished. No checkpoints were requested."
